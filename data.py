...
def _fetch(path, name):
    if name == 'train':
        token_file = os.path.join(path, 'train_tokens.mat')
        count_file = os.path.join(path, 'train_counts.mat')
    elif name == 'valid':
        token_file = os.path.join(path, 'valid_tokens.mat')
        count_file = os.path.join(path, 'valid_counts.mat')
    else:
        token_file = os.path.join(path, 'test_tokens.mat')
        count_file = os.path.join(path, 'test_counts.mat')
    tokens = scipy.io.loadmat(token_file)['tokens'].squeeze()
    counts = scipy.io.loadmat(count_file)['counts'].squeeze()
    if name == 'test':
        token_1_file = os.path.join(path, 'test_h1_tokens.mat')
        count_1_file = os.path.join(path, 'test_h1_counts.mat')
        token_2_file = os.path.join(path, 'test_h2_tokens.mat')
        count_2_file = os.path.join(path, 'test_h2_counts.mat')
        tokens_1 = scipy.io.loadmat(token_1_file)['tokens'].squeeze()
        counts_1 = scipy.io.loadmat(count_1_file)['counts'].squeeze()
        tokens_2 = scipy.io.loadmat(token_2_file)['tokens'].squeeze()
        counts_2 = scipy.io.loadmat(count_2_file)['counts'].squeeze()
